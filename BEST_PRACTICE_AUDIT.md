...
| **A.M-R-02 / P-5 — Costing-rollup migration** | Done. ~370 lines of inline rollup logic moved verbatim from `core/views/contract_budget.py` and `core/views/hc_claims.py` into `core/services/costing_rollups.py`. The service now exports `compute_project_committed_billed`, `hc_committed_amounts`, `hc_invoiced_amounts`, and `compute_staff_hours_allocation_amount` alongside the existing per-item primitives. Both views keep legacy aliases at the top of the module so external imports keep working. **Bonus:** caught and fixed a latent `NameError` / `AttributeError` in CB's billed-side snap loop (`item_name_to_costing` was deleted by A.M-C-13 but the second use site at line 521 was missed; the loop also reverted to `.values()` dict access). 12 deterministic regression tests in `core/tests/test_costing_rollups.py` pin every cell of committed_dict, billed_dict, and the HC-claim shape so future changes can't drift. |
| **A.M-R-03 / A.M-H-02 — Bill-allocation queryset helpers** | Done. Added `BillAllocationsQuerySet.direct_cost_lines()` and `.counts_toward_hc_invoiced()` + a `BillsQuerySet.direct_cost()` shorthand. The canonical Q-clause `bill_type IN (0,1) OR (bill_type=2 AND allocation_type=1)` now lives in exactly one place — the queryset method. All callers (`costing_rollups.py` × 3 and `contract_budget.py` × 1) migrated. 4 new tests in `core/tests/test_costing_rollups.py` lock the contract directly (16/16 pass). |
| **B.V-H-02 — Bill-status filter sweep** | Done. Migrated every active-code-path `bill_status__in=[...]` literal to the matching `BillsQuerySet` method (`pending_approval`, `approved_for_xero`, `in_xero`). Magic numbers (`bill_status = 0`, `= 1`, etc.) replaced with named `Bills.STATUS_*` constants. Touched `core/views/bills.py`, `core/views/bills_global.py`, `core/views/dashboard.py`, `construction/views/claims.py`, plus a defensive `getattr(Bills, 'STATUS_UNALLOCATED')` fallback to a non-existent constant in `bills.upload_bill` (replaced with `STATUS_CREATED`). The only remaining literal `bill_status=2` is in the dead `get_bills_by_supplier` view, which references renamed fields and isn't reachable from any UI — flagged for dead-code deletion separately. |
| **A.M-H-01 — `formulas.Committed()` dead code** | Deleted. The function was imported in `core/views/{bills,documents,main}.py` and `construction/views/claims.py` but called by zero of them; all four imports stripped, function body removed from `core/formulas.py`. |
| **A.M-C-01 — `bill_status=2` overloaded (approved vs sent-to-Xero)** | Done. The Direct send path in `bills_global._send_bill_to_xero_core` (workflow=`'direct'`) already lands the row on `STATUS_SENT_TO_XERO` (3) instead of `STATUS_APPROVED` (2); the Approvals path graduates `APPROVED → SENT_TO_XERO` and `PO_APPROVED_BILL_FOR_PAYMENT → PO_SENT_TO_XERO`. The dashboard "ready to send" tile and `get_approved_bills_for_xero` now both go through `BillsQuerySet.approved_for_xero()`, which carries `bill_xero_id__isnull=True` *inside* the helper — so even a future caller that forgets the guard still gets safe behaviour. Locked with 10 deterministic regression tests in `core/tests/test_bill_status_querysets.py` covering every status bucket plus the structural invariant `approved_for_xero ∩ in_xero = ∅`. |
| **A.M-H-16 — legacy `post_bill` / `test_xero_bill` Xero endpoints** | Deleted. Both views were stuck behind `raise NotImplementedError` (no UI caller, no OAuth2 plumbing) but still carried the historical `bill_status = STATUS_APPROVED` write after a Xero send — exactly the overloaded-status bug A.M-C-01 flagged. Removed the function bodies, the URL routes (`/post_bill/`, `/test_xero_bill/`), the `core/views/__init__` re-exports, the now-unused `requests` and `urljoin` imports in `bills.py`, and the legacy section of the file's header docstring. The OAuth2 send path lives in `bills_global._send_bill_to_xero_core`. |
