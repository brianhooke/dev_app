...
from .models import Quote_allocations, Invoice_allocations
...
def Committed():
    ...
    # invoice_type=1 OR (invoice_type=2 AND allocation_type=1)
    invoice_sum = (
        Invoice_allocations.objects
        .filter(
            models.Q(invoice_pk__invoice_type=1) |
            (models.Q(invoice_pk__invoice_type=2) & models.Q(allocation_type=1))
        )
