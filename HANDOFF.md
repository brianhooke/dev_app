...
**Last updated:** 28 May 2026 (v258 — staged for deploy: 4 user-reported fixes — allocations scrolling, PO PDF iframe, PO category ordering, PO claim submit 500)
...
- **Local code-state version**: `v258` — staged for next deploy. v253 + v254 + v255 + v256 + v257 + v258 changes folded into one zip:
  - **v258** (no migration): four user-reported regressions/bugs in the quote + PO contractor flows. (1) Allocations table (`allocations_layout.html`) now bounds the scrollable area at `max-height: 240px` with a sticky thead — previously the inner wrapper had `overflow: hidden` and no height cap so adding many allocation rows pushed rows outside the parent flex bounds with no scrollbar; affected sections: Bills, Quotes, HC Variations, HC Claims (any section using the shared layout). (2) `view_po_pdf_by_unique_id` is now decorated with `@xframe_options_sameorigin`. Production sets `X_FRAME_OPTIONS = 'DENY'` (settings/production_aws.py), which was blocking the same-origin iframe on `/po/<unique_id>/` from embedding `/po/<unique_id>/pdf/`; contractors saw a "broken link" icon. SAMEORIGIN is exactly the right policy here — the parent landing page is on the same origin and arbitrary third-party sites still cannot frame the PDF. (3) Both the PO PDF (`send_po_email`) and the public landing page (`view_po_by_unique_id`) plus the staff-facing pivot (`get_po_table_data_for_invoice`) now order rows by `(category.order_in_list, costing.order_in_list, costing_pk)`. The previous behaviour relied on PG row order without any `ORDER BY`, so the PDF and the input table could end up in *opposite* order — exactly what the user reported. `Po_order_detail.objects.create()` calls in `send_po_email` also iterate the sorted list so newly-created DB rows pick up auto-incrementing PKs in the correct sequence. (4) `submit_po_claim` previously called `Bills.objects.select_for_update().get(...)` and `.filter(...).first()` *outside* any `transaction.atomic()` block, which raises `TransactionManagementError` and 500s every contractor submit — the whole find-or-create + delete + recreate flow now lives inside one atomic block. The email-send is intentionally outside the transaction so a flaky SMTP doesn't block the DB write. (5) `loadAllocations` in `allocations_layout.js` now logs every success/failure path with structured `[AllocationsManager]` prefixes and wraps the `populateAllocationsTable` call in `try/catch` so a renderer exception cannot leave the table half-rendered with the error swallowed by the jQuery success handler — added to help diagnose a still-open user-reported issue ("clicking a saved quote in view mode shows no allocations"); next-time repro now leaves a console trail.
  - **v257** (no migration): HC variations and HC variation allocations may now be negative (scope reductions). Two minimal frontend edits — server endpoints, models (`Hc_variation.amount`, `Hc_variation_allocations.amount` are vanilla `DecimalField` with no validators), and the rollup math (`_compute_project_export_totals` adds variations as a signed sum into `revenue_receivable`) all already supported negatives. (1) `core/templates/core/hc_variations.html::saveNewVariation` previously hard-blocked `amount <= 0` with an alert; replaced with a near-zero check (`Math.abs(amount) < 0.01`) so $0 variations are still rejected but negatives go through. The reactive `validateNewVariation` (Save-button enabler) at line ~593 was already negative-friendly via `Math.abs`. (2) `core/templates/core/contract_budget.html` line 688 had `hasHcVariations = (item.hc_variation_amount || 0) > 0` which suppressed the click-to-expand HC-variation breakdown row for negative-net items; relaxed to `Math.abs(...) >= 0.01`. Construction-type projects (qty × rate) are unaffected: negatives simply require the user to enter qty *or* rate negative, the existing `qty * rate = amount` math handles signs. Manual variation seeded for `Onyx Factory (Subbie)` (project_pk=34, status=execution): `Hc_variation` total `-32000.00` dated `2026-05-28`, six `Hc_variation_allocations` (Mezz/Flooring -2754, Mezz/LG Framing -2222, Mezz/Rondo S/I -5355, Mezz/Gyprock -9517.5, Mezz/Painting -6345, Labour/Mezz -5806.5), notes "Remove Downstairs Office".
